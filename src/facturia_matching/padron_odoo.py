...
from facturia_matching.config import PADRON_ODOO_LIMIT
...
_PADRON_ODOO_CACHE: Optional[List[Dict[str, Any]]] = None


def reset_padron_odoo_cache() -> None:
    global _PADRON_ODOO_CACHE
    _PADRON_ODOO_CACHE = None
...
def build_padron_rows_from_odoo(limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    rubro_names = _fetch_rubro_names(category_vals)
...
def get_padron_odoo_cached() -> List[Dict[str, Any]]:
    global _PADRON_ODOO_CACHE
    if _PADRON_ODOO_CACHE is None:
        try:
            _PADRON_ODOO_CACHE = build_padron_rows_from_odoo()
        except Exception:
            _PADRON_ODOO_CACHE = []
    return _PADRON_ODOO_CACHE
