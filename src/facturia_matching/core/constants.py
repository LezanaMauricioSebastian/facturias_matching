...
def output_headers_for_profile(odoo_profile: Optional[Any] = None) -> List[str]:
    from facturia_matching.odoo.env import supports_rubro_field

    keys = ui_header_keys()
    if supports_rubro_field(odoo_profile):
        return keys
    return [k for k in keys if k != "x_studio_category"]
